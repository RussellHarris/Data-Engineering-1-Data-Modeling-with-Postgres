...
def process_log_file(cur, filepath):
    """
    Processes all files in the log data filepath.
    
    Args:
        cur (conn.cursor()): Allows Python code to execute PostgreSQL command in a database session..
        filepath (str): Log Data filepath.
    ...
    for index, row in df.iterrows():
        ...
        # Testing
        #cur.execute(logdata_table_insert, row)

        
def process_data(cur, conn, filepath, func):
    """
    Wrapper function to process files.
    
    Args:
        cur (conn.cursor()): Allows Python code to execute PostgreSQL command in a database session..
        conn (psycopg2.connect()): PostgreSQL driver.
